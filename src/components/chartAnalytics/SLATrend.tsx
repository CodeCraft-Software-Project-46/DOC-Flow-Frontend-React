...
export default function SLATrend() {
  ...
  const hasData = Array.isArray(TREND_DATA) && (TREND_DATA as Array<{ compliance?: number | null }>).some((d) => typeof d.compliance === "number" && d.compliance > 0);
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm p-6">
      ...
      <div className="min-h-[160px]">
        {loading ? (
          <div className="flex items-center justify-center min-h-[160px] text-xs text-slate-400">Loading chart...</div>
        ) : !hasData ? (
          <div className="flex items-center justify-center min-h-[160px] text-xs text-slate-400">No data available</div>
        ) : (
