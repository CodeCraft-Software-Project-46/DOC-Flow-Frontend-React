      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-[1100px] max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="bg-red-600 px-8 py-6 flex justify-between items-center">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-red-100 text-sm mt-1">{items.length} task{items.length !== 1 ? 's' : ''} need{items.length !== 1 ? '' : 's'} attention</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="overflow-auto flex-1 bg-slate-50">
          {items.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-64 text-slate-500">
              ...
              <p className="text-sm mt-1">All tasks are on track and within SLA</p>
            </div>
          ) : (
                ...
                {items.map((item, i) => (
                  ...
                  >
                    <td className="px-8 py-5 text-sm">
                      ...
                      <div className="text-xs text-slate-500 mt-1">ID: {item.task_id}</div>
                    </td>
                     <td className="px-8 py-5 text-sm">
        ...
        {items.length > 0 && (
          <div className="bg-gradient-to-r from-red-50 to-orange-50 border-t-2 border-red-200 px-8 py-4 flex justify-between items-center">
            <p className="text-sm text-slate-600">Total: <span className="font-bold text-red-600">{items.length}</span> task{items.length !== 1 ? 's' : ''} overdue</p>
            <button
              onClick={onClose}
