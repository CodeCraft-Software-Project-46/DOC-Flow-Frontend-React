      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-[1100px] max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="bg-blue-600 px-8 py-6 flex justify-between items-center">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-2xl font-bold text-white">Running Documents</h2>
              <p className="text-blue-100 text-sm mt-1">{items.length} active document{items.length !== 1 ? 's' : ''}</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="overflow-auto flex-1 bg-slate-50">
          {items.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-64 text-slate-500">
              ...
              <p className="text-sm mt-1">All documents are in their final stages</p>
            </div>
          ) : (
                ...
                {items.map((item, i) => (
                  ...
                  >
                    <td className="px-8 py-5 text-sm">
                      ...
                      <div className="text-xs text-slate-500 mt-1">ID: {item.document_id}</div>
                    </td>
                    <td className="px-8 py-5 text-sm">
